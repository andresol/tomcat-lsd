...
# Optimized for use with version 3.5.4 of the Wrapper.

# Check NODE_ID
if [ ! $(echo $NODE_ID | grep ^[0-9]*$) ]; then
	echo "You must define NODE_ID environment variable before calling this script"
	echo "This NODE_ID must be numeric. Example : export NODE_ID=1"
	exit 1
fi

# Check environment
if [ ! -n "$JAVA_HOME" ]; then
	echo "You must define JAVA_HOME environment variable before calling this script"
	exit 1
fi
if [ ! -n "$JVM_INIT_MEMORY" ]; then
	echo "You must define JVM_INIT_MEMORY environment variable in your startup script"
	exit 1
fi
if [ ! -n "$JVM_MAX_MEMORY" ]; then
	echo "You must define JVM_MAX_MEMORY environment variable in your startup script"
	exit 1
fi
if [ ! -n "$JVM_MAX_PERM_SIZE" ]; then
	echo "You must define JVM_MAX_PERM_SIZE environment variable in your startup script"
	exit 1
fi
...
export WRAPPER_HOME=$INSTALL_DIR/wrapper-linux-x86-64-3.5.16
export JNDI_PROVIDER_PATH=file:///data/shared_disk/jndi/provider-pprod.xml
export TOMCAT_HTTP_PORT=`expr 8080 + $NODE_ID - 1`
export TOMCAT_AJP_PORT=`expr 8200 + $NODE_ID`
export TOMCAT_AJP_JVMROUTE=${HOSTNAME_ALIAS}.${TOMCAT_ALIAS}_${NODE_ID}
export TOMCAT_SSL_PORT=`expr 8300 + $NODE_ID`
export TOMCAT_SHUTDOWN_PORT=`expr 8400 + $NODE_ID`
export JAVA_JMX_PORT=`expr 5000 + $NODE_ID`
export JAVA_DEBUG_PORT=`expr 8500 + $NODE_ID`
export WRAPPER_DUMP_PORT=`expr 7800 + $NODE_ID`
echo "Network ports, aliases and memory for this instance :"
echo "Tomcat http -> $TOMCAT_HTTP_PORT"
echo "Tomcat ajp -> $TOMCAT_AJP_PORT"
echo "Tomcat jvmRoute to support load-balancing via ajp -> $TOMCAT_AJP_JVMROUTE"
echo "Tomcat ssl -> $TOMCAT_SSL_PORT"
echo "Tomcat shutdown -> $TOMCAT_SHUTDOWN_PORT"
...
if [ ! -d $CATALINA_BASE ]; then
	...
	cp -r $INSTALL_DIR/tomcat_template $CATALINA_BASE
	echo "Created!"
fi

# Copy mandatory default web.xml from CATALINA_HOME/conf to CATALINA_BASE/conf
if [ ! -d $CATALINA_BASE/conf ]; then
	mkdir $CATALINA_BASE/conf
fi
cp -f $CATALINA_HOME/conf/web.xml $CATALINA_BASE/conf

# Application
APP_NAME="${CATALINA_INSTANCE}"
APP_LONG_NAME="Tomcat ${CATALINA_INSTANCE} Wrapper Application"

# Wrapper
WRAPPER_CMD="$WRAPPER_HOME/bin/wrapper"
WRAPPER_CONF="$CATALINA_SHARED/conf/wrapper.conf"
...
#  before returning.  For example, to delay the invocation of followingzerpforfait
